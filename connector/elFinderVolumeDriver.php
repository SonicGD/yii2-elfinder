...
abstract class elFinderVolumeDriver
{
    ...
    public function tree($hash = '', $deep = 0, $exclude = '')
    {
        ...
        $dirs = $this->gettree($path, $deep > 0 ? $deep - 1 : $this->treeDeep - 1,
    ...
    public function resize($hash, $width, $height, $x, $y, $mode = 'resize', $bg = '', $degree = 0)
    {
        ...
        switch ($mode) {
            ...
            case 'crop':
                ...
                break;
            case 'fitsquare':
                $result = $this->imgSquareFit($path, $width, $height, 'center', 'middle',
    ...
    protected function updateCache($path, $stat)
    {
        ...
        if ($this->options['utf8fix'] && $this->options['utf8patterns'] && $this->options['utf8replace']) {
            $stat['name'] = json_decode(str_replace($this->options['utf8patterns'], $this->options['utf8replace'],
    ...
    protected function createTmb($path, $stat)
    {
        ...
        if ($s[0] <= $tmbSize && $s[1] <= $tmbSize) {
            $result = $this->imgSquareFit($tmb, $tmbSize, $tmbSize, 'center', 'middle', $this->options['tmbBgColor'],
                'png');
        } else {
