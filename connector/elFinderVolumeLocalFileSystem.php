    ...
    protected function configure()
    {
        ...
        if ($root['read'] && !$this->tmbURL && $this->URL) {
            if (strpos($this->tmbPath, $this->root) === 0) {
                $this->tmbURL = $this->URL . str_replace(DIRECTORY_SEPARATOR, '/',
        ...
        if (!empty($this->options['quarantine'])) {
            if (is_dir($this->options['quarantine'])) {
                ...
                if ((!is_dir($this->quarantine) && !$this->_mkdir($this->root,
                            $this->options['quarantine'])) || !is_writable($this->quarantine)
                ) {
    ...
    protected function _extract($path, $arc)
    {
        ...
        if ($this->quarantine) {
            ...
            if (count($ls) == 1) {
                ...
                $name = basename($path);
                if (preg_match('/\.((tar\.(gz|bz|bz2|z|lzo))|cpio\.gz|ps\.gz|xcf\.(gz|bz2)|[a-z0-9]{1,4})$/i', $name,
                    $m)) {
